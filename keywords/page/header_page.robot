...
Verify the user is not loged in
    Wait Until Page Contains    ${header_page_translation.login_text}    timeout=${TIME_OUT}     
Log in user
    Wait Until Element Is Visible    ${HEADER_LOGIN_BUTTON}    timeout=${TIME_OUT}
    Click Element    ${LOCATOR_POPUP_BUTTON}
...
Verify log in successful
    Wait Until Page Contains    ${header_page_translation.hi_text}    timeout=${TIME_OUT}
Search for items 
